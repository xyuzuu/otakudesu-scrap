...
export const getAnime = async (requestUrl: string, q: string, ext = false):
   Promise < Anime[] > => {
      const response = await phin({ url: new URL(getSearchAnimeEndpoint(q), requestUrl) });
      ...
      return $('ul.chivsrc > li').map((_, element) => ({
         ...
         slug: $(element).find('h2 > a').attr('href')?.split('/').filter((s) => s.length).pop() as string,
         extra: () => getExtraAnime(requestUrl, $(element).find('h2 > a').attr('href')?.split('/').filter((s) => s.length).pop() as string) as Promise < ExtraAnime > ,
      })).toArray < Anime > ();
   };
