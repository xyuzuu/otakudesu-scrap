...
export const getOngoingList = async (requestUrl: string):
   Promise < OngoingAnime[] > => {
      const response = await phin({ url: new URL(ListEndpoint.ongoing, requestUrl) });
      ...
      return $('.venz > ul > li').map((_, element) => ({
         ...
         releaseAt: OtakUtil.resolveReleaseDate($(element).find('.detpost > .newnime').text().trim() + ' ' + $(element).find('.detpost > .epztipe').text().trim()),
         image: $(element).find('.detpost > .thumb > a > .thumbz > img').attr('src') as string,
         name: $(element).find('.detpost > .thumb > a > .thumbz > .jdlflm').text().trim(),
         url: $(element).find('.detpost > .thumb > a').attr('href') as string,
         slug: ($(element).find('.detpost > .thumb > a').attr('href') as string).split('/').filter((s) => s.length).pop() as string,
